...
public class Database {
    ...
    static Map<String, Distance> loadData() {
        Map<String, Distance> distanceMap = new ConcurrentHashMap<>();
    ...
    public synchronized static double calculateTotalDistance(List<String> distances) throws InvalidDistanceException {
        Map<String, Distance> distanceMap = loadData();
        double total = 0;
        for (String id : distances) {
            if (!distanceMap.containsKey(id)) {
                throw new InvalidDistanceException(
                        new StringBuilder().append("Encountered Invalid Distance -> ")
                        .append(id).toString());
            }
            //System.out.println("Getting distance : " + id + " : " + distanceMap.get(id).getDistance());
